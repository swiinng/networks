# networks
networks resources and docs


## resources
- beej's guide to net programming https://www.beej.us/guide/bgnet ([pdf](./resources/Beej’s%20Guide%20to%20Network%20Programming%20Using%20Internet%20Sockets.pdf))
- ip rfc 791 https://datatracker.ietf.org/doc/html/rfc791

## libraries
- libevent non-blocking https://libevent.org/libevent-book/

## tools and utils

### transport / ip
- wireshark _protocol analyzer_ https://www.wireshark.org/
- tshark _wireshark cli_ https://tshark.dev/
- tcpdump https://www.tcpdump.org/
- iproute2mac _ip utils_ https://github.com/brona/iproute2mac
- nc _netcat tcp/udp server/client_ https://linux.die.net/man/1/nc 
- socat _bidirectional socket relay_ https://man7.org/linux/man-pages/man1/socat.1.html

### resolution
- arp _address resolution protocol / lan_
- dig _dns_ https://linux.die.net/man/1/dig
- nslookup _simple dns lookup_

### interface / network
- ifconfig _interface config_ https://man7.org/linux/man-pages/man8/ifconfig.8.html
- nmap _network scan_ https://nmap.org/
- networkSetup

### application
- curl _data transfer_ https://curl.se/
- httpie _api testing_ https://httpie.io/

### stats
- netstat _network stats_ https://linux.die.net/man/8/netstat
- networkQuality _network responsiveness test_
- nettop _network monitor_

### routing
- traceroute https://linux.die.net/man/8/traceroute
- mtr _my trace route_ https://www.cloudflare.com/learning/network-layer/what-is-mtr/